...
class EasysocialApiResourceSociallogin extends ApiResource
{
	...
	{		
		...
		$this->provider = $app->input->get('provider', '' , 'STRING');
		...
		if( $this->accessToken ) {
			...
			if ($this->provider == 'google') {
				$objFbProfileData = $this->jfbGetUserGoogle( $this->accessToken );
				$userId = $this->jfbGetUserFromMap( $objFbProfileData );
			} 
			
			if ($this->provider == 'facebook') {
				$objFbProfileData = $this->jfbGetUserFacebook( $this->accessToken );
				$userId = $this->jfbGetUserFromMap( $objFbProfileData );		
			}
	...
	function jfbRegister( $objFbProfileData ) {
	 ...
	 * function to get user from #_jfbconnect_user_map
	 */
	public function jfbGetUserGoogle( $accessToken )
	{
	 ...
	 * function to get user from #_jfbconnect_user_map
	 */	 
	public function jfbGetUserFacebook( $accessToken )
	{
	...
	public function create_field_arr($fields,$post)
	{
		 ...
		 if(!empty($post['avatar']))
		{
			...
			$phto_obj = $upload_obj->ajax_avatar($post['avatar']);
			$avtar_pth = $phto_obj['temp_path'];
			$avtar_scr = $phto_obj['temp_uri'];
			$avtar_typ = 'upload';
			$avatar_file_name = $post['avatar']; 
		}
