<?php
/**
 * @version   $Id: PhpQueryScanStrategy.php 30621 2017-08-01 08:15:37Z matias $
...
class RokBooster_Joomla_PhpQueryScanStrategy extends RokBooster_Joomla_AbstractStrategy
{
	...
	protected function identifyInlineScripts()
	{
		...
		$tags        = pq($this->prefix . 'script:not([src]):not([type=application/json])', $this->document);
		foreach ($tags as $tag) {
	...
	protected function populateInlineScripts()
	{
		if ($this->options->inline_js) {
			...
			$outputblocks = array();
			pq($this->prefix . 'script:not([src]):not([type=application/json])', $this->document)->remove();
			...
			$script_block = pq($this->prefix . 'script:not([src]):not([type=application/json])', $this->document);
			if ($script_block) {
