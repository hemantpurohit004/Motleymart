...
class GantryDropdownLayout extends AbstractRokMenuLayout
{
    ...
    public function stageHeader()
    {
		...
		'menuHoverColor'=>$gantry->get('body-linkcolor'),
		'menu-border-width'     => $gantry->get('menu-border-width',     '3px'),
		'menu-top-border-color'     => $gantry->get('menu-top-border-color',     '#ea313c'),
		'menu-bottom-border-color'     => $gantry->get('menu-bottom-border-color',     '#bb2833'),
		...
		'menudropdown-active-hover-fontcolor'     => $gantry->get('menu-ropdown-active-hover-fontcolor',     '#ffffff'),
		...
		$gantry->addLess('menu.less', 'menu.css', 1, $menuVariables);

        // no media queries for IE8 so we compile and load the hovers
        if ($gantry->browser->name == 'ie' && $gantry->browser->shortver < 9){
            $gantry->addLess('menu-hovers.less', 'menu-hovers.css', 1, $menuVariables);
        }
    }
